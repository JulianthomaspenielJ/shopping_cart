...
import { ORDER, ENUMSTATUS, BOOKING, EVENT_CAT, STATUSVALUE, USER, FORMAT, PAGE } from '../../lib/const'
import { connect } from 'react-redux'
import moment from 'moment'
import { currentbooking } from './actions'
...
const CurrentBookings = (props) => {
  const { navigation, dispatch, userCurrentBooking, loading, page, t } = props
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      screenViewed(PAGE.PASTBOOKING)
      setCurrentBooking([])
      dispatch(currentbooking(ORDER.CURRENT, USER))
    })
    return unsubscribe
  }, [navigation])
    ...
    return bookingslot && bookingslot.map(function (item, index) {
      return (
        <View key={index} style={{ flexDirection: 'row' }}>
          ...
          <Text key={index} style={[ratingDetailsStyles.cookCuisineDetails, { paddingTop: 3, paddingLeft: 8, fontSize: 8, alignSelf: 'flex-start' }]}>&nbsp;({item.mealType ? item.mealType.name : item.name})</Text>
        </View>
      )
    })
  }
      ...
      <View key={index} style={[ratingDetailsStyles.cookDetailsContainer, { margin: 10, borderRadius: 30, padding: 23 }]}>
        ...
        <View style={[ratingDetailsStyles.cookContainerLine]}>
          <View style={{ paddingBottom: 12 }}>
            <View style={{ flexDirection: 'row', paddingLeft: 12 }}>
              <View style={{ flex: 1, flexDirection: 'row' }}>
                ...
                <Text ellipsizeMode='tail' numberOfLines={1} style={[ratingDetailsStyles.cookCuisineDetails, ratingDetailsStyles.ellips]}>{item.details &&
          ...
          <View style={{ flexDirection: 'row', paddingLeft: 12 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[orders.totalPrice, { textAlign: 'right' }]}>
                <Icon name='rupee' size={16} color={darkCyan} />&nbsp;{Math.round(item.totalAmount)}
              </Text>
            </View>
          </View>
        </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(withTranslation()(CurrentBookings))
