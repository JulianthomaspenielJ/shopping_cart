    ...
    <View style={[supportMenu.container, { backgroundColor: lightGrey, padding: 0 }]}>
      ...
      >
        ...
        <View>
          ...
          >
            <Text style={supportMenu.titleText}>{t('troubleshoot')}</Text>
          </View>
          <View style={[supportMenu.borderLine]} />
        </View>
      </View>
      {
        lang === 'en' && userType === COOK ? <WebView source={{ uri: `${apiUrl.TROUBLESHOOT}` }} /> : null
      }
      {
        lang === 'en' && userType !== COOK ? <WebView source={{ uri: `${apiUrl.TROUBLESHOOT}` }} /> : null
      }
      {
        lang === 'ta' && userType === COOK
          ? (
            <WebView source={{ uri: `${apiUrl.TROUBLESHOOT_TAMIL}` }} />
          ) : null
      }
      {
        lang === 'ta' && userType !== COOK ? <WebView source={{ uri: `${apiUrl.TROUBLESHOOT_TAMIL}` }} /> : null
      }
      {
        lang === 'hi' && userType === COOK ? <WebView source={{ uri: `${apiUrl.TROUBLESHOOT}` }} /> : null
      }
      {
        lang === 'hi' && userType !== COOK ? <WebView source={{ uri: `${apiUrl.TROUBLESHOOT}` }} /> : null
      }
...
export default (withTranslation()(Troubleshoot))
